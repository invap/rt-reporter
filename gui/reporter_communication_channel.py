...
class ReporterCommunicationChannel:
    def __init__(self, path, process_name):
        ...
        self.__process_thread = threading.Thread(
            target=self.__process_incoming_data, args=()
        )
    ...
    def get_count(self):
        return [
            self.__timed_events_count,
            self.__state_events_count,
            self.__process_events_count,
            self.__component_events_count,
        ]
    ...
    def __process_incoming_data(self):
        while True:
            ...
            if self.__stop_event.is_set():
                break
            buffer = self.__channel.stdout.read(
                self.__chn_conf.capacity * self.__chn_conf.max_pkg_size
            )
            pkgs = [
                buffer[i : i + self.__chn_conf.max_pkg_size]
                for i in range(0, len(buffer), self.__chn_conf.max_pkg_size)
            ]
            for pkg in pkgs:
                if self.__stop_event.is_set():
                    break
                ...
                match event_type:
                    ...
                    case 4:
                        ...
                        self.__event_logs_map[stripped_data_string] = open(
                            self.__files_path + "/" + stripped_data_string + "_log.csv",
                            "w",
                        )
                    case 5:
                        self.__component_events_count += 1
